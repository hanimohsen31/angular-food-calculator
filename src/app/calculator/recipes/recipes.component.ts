...
export class RecipesComponent implements OnInit {
    ...
    this.FoodDataService.getFoodData().subscribe({
      next: (res) => {
        ...
        this.recipesList = array
          ...
          .sort((a: any, b: any) =>
            (a.ShortFoodName || '').localeCompare(b.ShortFoodName || '')
          );
        this.applyFilter();
  ...
  onDocumentClick(event: any) {
    if (
      this.dropdownOpen &&
      !this.foodDropdown?.nativeElement.contains(event.target)
    ) {
      this.closeDropdown();
    }
  }
  ...
  getPickedQuantity(food: any) {
    let existed = this.recipeItems.find(
      (elm: any) => elm.FoodKey === food.FoodKey
    );
  ...
  copyRecipe(recipe: any) {
    ...
    this.formData.patchValue({
      ...
      Translation: `${recipe.Translation || ''} نسخة`,
    });
  }
